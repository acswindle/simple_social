from fastapi import FastAPI , Form, status, Depends, Cookie, UploadFile 
...
async def add_post( post_title : Annotated[str,Form()],
    ...
    post = UserPostId(user_id=user_id, 
...
async def user_login(username : Annotated[str,Form()],
    ...
    correct_password = pbkdf2_sha256.verify(password + user.salt,
                                            user.hash_password)
...
@app.post('/like')
async def upload_like(post_id:PostId,
                      request : Request,
                      user_id : int = Depends(oauth_cookie))->HTMLResponse:
...
async def get_comment_form(post_id:int,
                           request : Request,
                           user_id : int = Depends(oauth_cookie))->HTMLResponse:
...
async def post_comment_form(post_id:int,
                            post_text : Annotated[str,Form()],
...
async def hide_thread(post_id:int,
                     request:Request,
                     access_token : Annotated[str|None,Cookie()] = None)->HTMLResponse:
